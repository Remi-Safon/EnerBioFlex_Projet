<?php
	...
	try{
		...
		$requete = $bdd->query('SELECT id_article, titre, date_publication, prix, description, voie, photo, region, departement, ville FROM article 
		...
		while ($articles = $requete->fetch()) {
			if ($articles['id_article'] == $_GET['id_article']) {
				$user = $bdd->query('select nom from utilisateur join article using(id_utilisateur) where id_article='.$_GET['id_article']);
				$user = $user->fetch();
				echo "<body>
				<div id='box'>
